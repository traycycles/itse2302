...
$fieldData = test_input($_POST['fieldData']);
    ...
    if (!(preg_match('/^[a-zA-Z]*$/',$fieldData) ||
        ...
        {
            $sql = "DELETE FROM teams WHERE $fieldname=? LIMIT 1";
            if ($stmt = $db->prepare($sql))
            {
                ...
                {
?>              <p style="color: darkgreen; font-size: larger">DELETED!!!</p>
<?php
                }
                else
                {
?>                  <p style="color: red; font-size: larger">That data doesn't exist </p>
<?php           }
            }
